...
function WithdrawPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setError(''); setBusy(true); const form = new FormData(event.currentTarget)
    try { const amount = Number(form.get('amount')); await moveMoney('withdraw', amount); toast.success(`${money.format(amount)} withdrawn`); navigate(paths.account) }
    catch (caught) { setError(caught instanceof Error ? caught.message : 'Withdrawal failed.') } finally { setBusy(false) }
  }
  return <main className="mx-auto flex w-full max-w-md flex-1 flex-col justify-center px-5 py-12">{error && <Alert className="mb-4" variant="destructive"><AlertDescription>{error}</AlertDescription></Alert>}<MoneyForm action="withdraw" balance={selectedAccount ? money.format(selectedAccount.balance) : undefined} busy={busy} onSubmit={handleSubmit} /></main>
}
